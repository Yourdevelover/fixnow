...
<div style="max-width:560px; margin:0 auto;">
    ...
    <h1 style="font-size:22px; font-weight:600; margin-bottom:4px;">Edit Service</h1>
    <p style="color:#64748b; font-size:14px; margin-bottom:24px;">Mengubah data layanan <strong>{{ $service->service_name }}</strong>.</p>
        ...
        <form method="POST" action="/admin/services/{{ $service->id }}">
            ...
            <div style="margin-bottom:18px;">
                ...
                <textarea name="description" required rows="4"
                    style="width:100%; padding:10px 12px; border:1px solid #e2e8f0; border-radius:8px; font-size:14px; resize:vertical;">{{ old('description', $service->description) }}</textarea>
            </div>
            ...
            <div style="margin-bottom:24px;">
                ...
                <div style="position:relative;">
                    ...
                    <input type="number" name="base_price"
                        value="{{ old('base_price', $service->base_price) }}" required min="0"
                        style="width:100%; padding:10px 12px 10px 36px; border:1px solid #e2e8f0; border-radius:8px; font-size:14px;">
                </div>
            </div>

            {{-- Info slug --}}
            <div style="padding:10px 14px; background:#f8fafc; border-radius:8px; font-size:12px; color:#94a3b8; margin-bottom:20px;">
                Slug saat ini: <code>{{ $service->slug }}</code> — otomatis diperbarui saat nama diubah.
            </div>
